...
[![GoDoc](https://img.shields.io/badge/godoc-reference-blue.svg)](http://godoc.org/github.com/AnumaNetwork/dnsseeder)

DNSSeeder exposes a list of known peers to any new peer joining the Anuma network via the DNS protocol.

When DNSSeeder is started for the first time, it will connect to the anumad node
...
disk in a json file, so on subsequent start ups the anumad node specified with
...
- Launch a anumad node for the DNSSeeder to connect to
...
```bash
$ git clone https://github.com/AnumaNetwork/dnsseeder $GOPATH/src/github.com/AnumaNetwork/dnsseeder
$ cd $GOPATH/src/github.com/AnumaNetwork/dnsseeder
$ go install .
```
...
To create a working set-up where the DNSSeeder can provide IPs to anumad instances, set the following DNS records:
